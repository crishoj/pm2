...
module.exports = function ForkMode(God) {
  ...
  God.forkMode = function forkMode(pm2_env, cb) {
    ...
    if (pm2_env.expand_args || pm2_env.increment_var) { // HACK: expand_args somehow gets cleared on restart
      // expand variables in command args
      args = args.map(arg => arg.replace(/\$(\w+)/ig, (match, name) => {
        console.log("expand_arg: $%s => %s", name, pm2_env.env[name]);
        return pm2_env.env[name];
      }));
    }
